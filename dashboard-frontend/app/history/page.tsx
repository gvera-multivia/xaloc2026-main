...
import { Search, Calendar, CheckCircle2, ChevronLeft, ChevronRight, Download, X, Crown, Medal, Award, Trophy } from 'lucide-react';
...
export default function HistoryPage() {
  ...
  const isTopView = String(searchParams.get('view') || '').trim().toLowerCase() === 'top';
  ...
  useEffect(() => {
    ...
    const loadTop = async () => {
      ...
      try {
        ...
        setTopItems((globalRes.items || []) as TopUserItem[]);
        setTodayTopItems(((todayRes.items || []).slice(0, 5)) as TopUserItem[]);
        ...
        <div>
          ...
          <div className="mt-3 flex items-center gap-2">
            <Link
              href="/history"
              className={cn(
                "px-3 py-1.5 rounded-sm text-[10px] font-black uppercase tracking-[0.15em] border transition-all",
                !isTopView
                  ? "border-[rgba(108,77,255,0.35)] bg-[rgba(108,77,255,0.16)] text-foreground/90"
                  : "border-border/70 bg-[rgba(17,19,26,0.55)] text-muted-foreground/80 hover:text-foreground hover:border-[rgba(108,77,255,0.22)]"
              )}
            >
              Historial
            </Link>
            <Link
              href="/history?view=top"
              className={cn(
                "px-3 py-1.5 rounded-sm text-[10px] font-black uppercase tracking-[0.15em] border transition-all",
                isTopView
                  ? "border-[rgba(108,77,255,0.35)] bg-[rgba(108,77,255,0.16)] text-foreground/90"
