...
def test_fetch_candidates_accepts_orgt_diba_barcelona_city_for_redsara() -> None:
    adapter = RedsaraAdapter()
    ...
    assert len(out) == 1
    assert out[0]["idRecurso"] == 103842


def test_fetch_candidates_rejects_orgt_diba_barcelona_city_for_identificacion() -> None:
    adapter = RedsaraAdapter()
...
def test_build_payloads_maps_orgt_diba_barcelona_city_to_barcelona_destination(monkeypatch) -> None:
    adapter = RedsaraAdapter()
    async def _fake_docs(*args, **kwargs):
        return []

    monkeypatch.setattr("sites.adapters.redsara.get_required_client_documents", _fake_docs)

    payloads = __import__("asyncio").run(
            ...
            [
                {
                    ...
                    "cliente_domicilio": "PLA DELS CIRERERS",
                    "cliente_cp": "08042",
                    "FaseProcedimiento": "denuncia",
                    "SujetoRecurso": "JUAN RAMON GIMENEZ MANZANO",
                    "cliente_nif": "12345678Z",
                    "cliente_nombre": "JUAN RAMON",
                    "cliente_apellido1": "GIMENEZ",
                    "cliente_apellido2": "MANZANO",
                    "adjuntos": [],
                }
            ]
        )
    ...
    assert payloads[0]["destination_organism_code"] == "LA0006797"
